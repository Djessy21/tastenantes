  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label
          htmlFor="name"
          className="block text-sm font-medium text-[#5D4D40]"
        >
          Nom du plat
        </label>
        <input
          ...
          required
          className="mt-1 block w-full rounded-md border border-[#E8E1D9] px-3 py-2 focus:border-[#6B5D4F] focus:outline-none focus:ring-1 focus:ring-[#6B5D4F]"
        />
      </div>
      ...
      {error && <div className="text-[#E05D5D] text-sm">{error}</div>}

      <button
        type="submit"
        disabled={isSubmitting || !imageUrl}
        className="w-full bg-[#6B5D4F] text-white py-2 px-4 rounded-md hover:bg-[#5D4D40] disabled:bg-[#D2C8BC] disabled:cursor-not-allowed transition-colors"
      >
